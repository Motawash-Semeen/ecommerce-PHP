...
if (isset($_GET['status'])) {
    if (isset($_SESSION['role'])) {
      if ($_SESSION['role'] == 'user') {
        ...
        $sql_status = "UPDATE vendors SET vendor_status = '$satus' WHERE vendor_id = '$id'";
...
<div class="row">
    <div class="col-md-12">
        <div style="text-align:right; margin:auto;">
            <a href='index.php?add_vendor' class='btn btn-link btn-warning btn-just-icon edit'
                                        ...
                                        if ($result_data->num_rows > 0) {
                                            ...
                                            while ($row = $result_data->fetch_array()) {
                                                echo "<tr role='row' class='odd'>
                                                                ...
                                                                <td tabindex='0' class='sorting_1'>{$row['vendor_title']}</td>
                                                                <td tabindex='0' class='sorting_1'>
                                                                <img src='../img/vendor/{$row['vendor_img']}' alt='cat_img' width='100'>
                                                                </td>
                                                                <td tabindex='0' class='sorting_1'>
                                                                <a href='index.php?vendors&id={$row['vendor_id']}&status={$row['vendor_status']}' class='btn btn-info'>{$row['vendor_status']}</a>
                                                                </td>
                                                                
                                                                <td class='text-right'>
                                                                    <a href='index.php?id={$row['vendor_id']}&edit_vendor' class='btn  btn-warning '>
                                                                    EDIT
                                                                    </a>
                                                                    <a onClick=\"javascript: return confirm('Are you sure you want to delete this Item?'); \" href='index.php?id={$row['vendor_id']}&vendors&delete' class='btn btn-danger'>
                                                                    DELETE
                                                                    </a>
                                                                </td>
                                                            </tr>";
